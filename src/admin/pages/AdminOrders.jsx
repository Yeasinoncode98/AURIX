  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-extrabold text-[22px] tracking-[-0.03em] text-white">Orders</h1>
          <p className="text-[12px] text-muted mt-0.5">{orders.length} total orders</p>
        </div>
      </div>
      ...
      <div className="rounded-[8px] border border-[#1a1a1a] overflow-hidden"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[12px] min-w-[900px]">
            <thead>
              <tr className="border-b border-[#151515]">
                {['Order ID','Customer','Payment','Items','Amount','Status','Handled By','Actions','Cash Memo'].map(h => (
          ...
          {sorted.length === 0 && (
            <p className="text-center text-muted text-[12px] py-10">No orders found</p>
          )}
        </div>
      </div>
      ...
      {selected && (
        <OrderDrawer order={selected} onClose={() => setSelected(null)}
          onStatusChange={updateStatus} onMemo={() => { setMemoOrder(selected); setSelected(null) }}/>
      )}
...
function OrderRow({ order, onStatusChange, onView, onMemo }) {
  ...
  return (
    <tr className={`hover:bg-[#0d0d0d] transition-colors duration-100
      ...
      <td className="px-4 py-3">
        ...
        <p className="text-muted text-[10px] mt-0.5">
          {order.createdAt?.toDate
            ? order.createdAt.toDate().toLocaleDateString('en-BD', { day:'2-digit', month:'short', year:'numeric' })
            : '—'}
        </p>
      </td>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] font-semibold text-green-400">Delivery Fee Paid</span>
        </div>
        <p className="text-muted text-[10px] mt-0.5">{order.paymentLabel} · ৳{order.deliveryFee}</p>
      </td>

      {/* Items */}
      <td className="px-4 py-3">
        <p className="text-white font-semibold">{order.items?.length || 0} item(s)</p>
      ...
      <td className="px-4 py-3">
        {order.handledBy ? (
          <>
            <p className="text-white text-[11px] font-semibold">{order.handledBy.name}</p>
...
function OrderDrawer({ order, onClose, onStatusChange, onMemo }) {
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-[480px] z-[101] overflow-y-auto
                      border-l border-[#1a1a1a]"
        style={{ background: '#0a0a0a' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1a1a1a] sticky top-0"
          style={{ background: '#0a0a0a' }}>
          <div>
            <p className="text-[11px] uppercase tracking-wider2 text-muted">Order Details</p>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="rounded-[8px] border border-[#1a1a1a] p-4">
            <p className="text-[10px] uppercase tracking-wider2 text-muted mb-3">Update Status</p>
            ...
            {(order.items || []).map((item, i) => (
              <div key={i} className="flex items-center justify-between py-2
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted">Qty: {item.qty} × ৳{item.price?.toLocaleString()}</p>
                </div>
                <span className="text-[12px] font-bold text-white">৳{item.subtotal?.toLocaleString()}</span>
              </div>
            ))}
          </DrawerSection>
          ...
          <DrawerSection title="Summary">
            <DrawerRow label="Subtotal"     value={`৳${order.subtotal?.toLocaleString()}`}/>
            <DrawerRow label="Delivery Fee" value={`৳${order.deliveryFee?.toLocaleString()}`}/>
            <DrawerRow label="Total" value={`৳${order.totalAmount?.toLocaleString()}`} highlight/>
          </DrawerSection>
...
function CashMemoModal({ order, onClose }) {
  ...
  const [mode, setMode] = useState('customer') // 'customer' | 'both'

  const printCustomer = useReactToPrint({ content: () => customerRef.current })
  const printBoth     = useReactToPrint({ content: () => bothRef.current })
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/70 z-[200] flex items-center justify-center p-4"
        onClick={onClose}>
        <div className="relative w-full max-w-[620px] rounded-[8px] border border-[#1a1a1a] overflow-hidden
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a1a1a] flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Tab toggle */}
              <div className="flex rounded-[6px] overflow-hidden border border-[#222]">
                <button
                  onClick={() => setMode('customer')}
                  className={`px-3 py-1.5 text-[11px] font-semibold transition-all duration-150
                    ${mode === 'customer' ? 'bg-[#222] text-white' : 'bg-transparent text-muted hover:text-white'}`}>
                  Customer Copy
                </button>
                <button
                  onClick={() => setMode('both')}
                  className={`px-3 py-1.5 text-[11px] font-semibold transition-all duration-150 border-l border-[#222]
                    ${mode === 'both' ? 'bg-[#222] text-white' : 'bg-transparent text-muted hover:text-white'}`}>
                  Both Copies
                </button>
              </div>
              ...
              {/* Print button */}
              <button
                onClick={mode === 'customer' ? printCustomer : printBoth}
                className="flex items-center gap-2 px-4 py-2 bg-red text-white rounded-[6px]
                ...
                Print {mode === 'customer' ? 'Customer Copy' : 'Both Copies'}
              </button>
          ...
          {/* Info strip */}
          <div className={`px-5 py-2 text-[11px] flex items-center gap-2 flex-shrink-0
                           ${mode === 'both' ? 'bg-green-500/10 text-green-400' : 'bg-blue-500/10 text-blue-400'}`}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {mode === 'customer'
              ? '1 page — customer copy only. Switch to "Both Copies" to print company record too.'
              : '1 page — both copies on same paper. Cut along dashed line. Saves paper!'}
          </div>

          {/* Preview */}
          <div className="overflow-y-auto flex-1 bg-white">

            {/* Customer only — hidden when mode=both */}
            <div ref={customerRef} style={{ display: mode === 'customer' ? 'block' : 'none' }}>
              <MemoContent order={order} orderDate={orderDate} copy="Customer Copy" />
            </div>

            {/* Both copies — hidden when mode=customer */}
            <div ref={bothRef} style={{ display: mode === 'both' ? 'block' : 'none' }}>
              <MemoContent order={order} orderDate={orderDate} copy="Customer Copy" />
              {/* Dashed cut line */}
              <div style={{
                ...
                <span style={{ visibility: 'hidden', fontSize: 10, padding: '2px 12px' }}>spacer</span>
              </div>
              <MemoContent order={order} orderDate={orderDate} copy="Company Copy" />
            </div>
      ...
      <style>{`
        @media print {
          ...
          #print-both, #print-both * { visibility: visible !important; }
          #print-customer, #print-both {
...
/* ── Memo content (used for both copies) ── */
function MemoContent({ order, orderDate, copy }) {
  ...
  return (
    <div style={{ fontFamily: "'Courier New', monospace", padding: '20px 28px', background: '#fff', color: '#111', fontSize: 12 }}>
      {/* Header */}
      <div style={{ textAlign: 'center', marginBottom: 14 }}>
        <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: -1, color: '#C1121F' }}>AURIX</div>
        <div style={{ fontSize: 9, color: '#888', letterSpacing: 3, textTransform: 'uppercase', marginTop: 2 }}>
          Premium Headphones
        </div>
        <div style={{
          fontSize: 10, fontWeight: 800, marginTop: 8,
          color: isCompany ? '#fff' : '#C1121F',
          background: isCompany ? '#111' : '#fff',
          border: `1px solid ${isCompany ? '#333' : '#C1121F'}`,
          display: 'inline-block', padding: '2px 14px', borderRadius: 4,
      ...
      {/* Order info */}
      <MemoRow label="Order ID"   value={order.orderId || order.id} bold />
      <MemoRow label="Date"       value={orderDate} />
      <MemoRow label="Status"     value={(order.status || '').toUpperCase()} />
      ...
      <MemoRow label="Name"     value={order.customerName} />
      <MemoRow label="Phone"    value={order.customerPhone} />
      <MemoRow label="Address"  value={order.deliveryAddress} />
      <MemoRow label="Zone"     value={order.deliveryLabel} />
      ...
      <MemoRow label="Paid Via"        value={order.paymentLabel} />
      <MemoRow label="Sender Number"   value={order.senderNumber} />
      <MemoRow label="TRX ID"          value={order.trxId} mono />
      <MemoRow label="Delivery Charge" value={`৳${deliveryFee} — ALREADY PAID`} highlight />
      ...
      {(order.items || []).map((item, i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between',
          ...
          <span style={{ fontWeight: 700, color: '#111' }}>৳{(item.subtotal ?? (item.price * item.qty))?.toLocaleString()}</span>
        </div>
      ))}
      ...
      <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'#22a861', marginBottom:8, fontWeight:600 }}>
        <span>Delivery Fee — Already Paid ✓</span>
        <span>৳{deliveryFee?.toLocaleString()}</span>
      </div>
      {/* COD box */}
      <div style={{
        display:'flex', justifyContent:'space-between', alignItems:'center',
        padding:'8px 12px', borderRadius:4, background:'#f5f5f5', border:'2px solid #111',
      }}>
        <div>
          <div style={{ fontSize:9, color:'#888', textTransform:'uppercase', letterSpacing:1 }}>
            Customer Pays at Door
          </div>
          <div style={{ fontSize:9, color:'#aaa' }}>Delivery ৳{deliveryFee} already paid</div>
        </div>
        <div style={{ fontSize:20, fontWeight:900, color:'#C1121F' }}>
          ৳{subtotal?.toLocaleString()}
        </div>
      </div>
      <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#999', marginTop:6 }}>
        <span>Full Order Value</span><span>৳{totalAmount?.toLocaleString()}</span>
      </div>

      {/* Footer */}
      <div style={{ textAlign: 'center', marginTop: 16, fontSize: 10, color: '#555' }}>
        Thank you for choosing AURIX · aurix.com
      </div>
    </div>
  )
}
...
function MemoRow({ label, value, bold, mono, highlight }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4,
                  fontSize: 12, alignItems: 'flex-start', gap: 8 }}>
      <span style={{ color: '#666', flexShrink: 0 }}>{label}</span>
      <span style={{
        color: highlight ? '#22c55e' : bold ? '#fff' : '#ccc',
  ...
  return (
    <div className="rounded-[8px] border border-[#1a1a1a] overflow-hidden">
      <div className="px-4 py-2.5 border-b border-[#1a1a1a] bg-[#0d0d0d]">
        <p className="text-[10px] uppercase tracking-wider2 text-muted font-semibold">{title}</p>
      </div>
