...
//         onClick={onClose}>
//         <div className="relative w-full max-w-[600px] rounded-[8px] border border-[#1a1a1a]
...
//               {/* Toggle: Customer / Both */}
//               <div className="flex rounded-[6px] overflow-hidden border border-[#2a2a2a]">
//                 <button onClick={() => setMode('customer')}
//                   className={`px-3 py-1.5 text-[11px] font-semibold transition-colors duration-150
//                     ${mode==='customer' ? 'bg-[#2a2a2a] text-white' : 'text-muted hover:text-white'}`}>
//                   Customer Only
//                 </button>
//                 <button onClick={() => setMode('both')}
//                   className={`px-3 py-1.5 text-[11px] font-semibold border-l border-[#2a2a2a]
//                               transition-colors duration-150
//                     ${mode==='both' ? 'bg-[#2a2a2a] text-white' : 'text-muted hover:text-white'}`}>
//                   Both Copies
//                 </button>
//               </div>

//               {/* Print */}
//               <button onClick={mode==='customer' ? printCustomer : printBoth}
...
//               <button onClick={onClose} className="text-muted hover:text-white ml-1">
...
//             </svg>
//             {mode==='customer'
//               ? 'Prints 1 page — customer copy only. Switch to "Both Copies" to include company record.'
//               : 'Prints 1 page — both copies together. Cut along dashed line. Saves paper!'}
//           </div>
...
//   const S = {
//     page:    { fontFamily:"'Courier New', Courier, monospace", background:'#ffffff', color:'#111111',
//                padding:'24px 32px', fontSize:12 },
//     center:  { textAlign:'center' },
//     brand:   { fontSize:26, fontWeight:900, color:'#C1121F', letterSpacing:-1 },
//     sub:     { fontSize:9,  color:'#555555', letterSpacing:3, textTransform:'uppercase', marginTop:2 },
//     badge:   { display:'inline-block', marginTop:8, padding:'2px 14px', borderRadius:4,
...
//     secTitle:{ fontSize:9, color:'#555555', textTransform:'uppercase',
//                letterSpacing:1.5, marginBottom:5, fontWeight:700 },
//     row:     { display:'flex', justifyContent:'space-between', alignItems:'flex-start',
//                marginBottom:4, gap:8 },
//     lbl:     { color:'#555555', flexShrink:0, fontSize:11 },
//     val:     { color:'#111111', fontWeight:600, textAlign:'right', wordBreak:'break-all', fontSize:11 },
//     valMono: { color:'#111111', fontWeight:700, textAlign:'right', fontFamily:'monospace',
//                letterSpacing:'0.06em', fontSize:11, wordBreak:'break-all' },
//     valGreen:{ color:'#166534', fontWeight:700, textAlign:'right', fontSize:11 },
//     codBox:  { display:'flex', justifyContent:'space-between', alignItems:'center',
//                padding:'10px 14px', borderRadius:4,
//                background:'#f3f3f3', border:'2px solid #111111', marginTop:10 },
//     codLbl:  { fontSize:9, color:'#555555', textTransform:'uppercase', letterSpacing:1 },
//     codSub:  { fontSize:9, color:'#888888', marginTop:2 },
//     codAmt:  { fontSize:22, fontWeight:900, color:'#C1121F' },
//     footer:  { textAlign:'center', marginTop:14, fontSize:9, color:'#888888' },
//   }
...
//       <div style={S.row}><span style={S.lbl}>Order ID</span><span style={{...S.val, fontWeight:800}}>{order.orderId||order.id}</span></div>
//       <div style={S.row}><span style={S.lbl}>Date</span><span style={S.val}>{orderDate}</span></div>
//       <div style={S.row}><span style={S.lbl}>Status</span><span style={S.val}>{(order.status||'pending').toUpperCase()}</span></div>

//       <div style={S.hrDash}/>

//       {/* Customer */}
//       <div style={S.secTitle}>Customer Details</div>
//       <div style={S.row}><span style={S.lbl}>Name</span>   <span style={S.val}>{order.customerName||'—'}</span></div>
//       <div style={S.row}><span style={S.lbl}>Phone</span>  <span style={S.val}>{order.customerPhone||'—'}</span></div>
//       <div style={S.row}><span style={S.lbl}>Address</span><span style={{...S.val, maxWidth:'60%', lineHeight:1.5}}>{order.deliveryAddress||'—'}</span></div>
//       <div style={S.row}><span style={S.lbl}>Zone</span>   <span style={S.val}>{order.deliveryLabel||'—'}</span></div>

//       <div style={S.hrDash}/>

//       {/* Delivery payment */}
//       <div style={S.secTitle}>Advance Payment — Delivery Fee</div>
//       <div style={S.row}><span style={S.lbl}>Paid Via</span>      <span style={S.val}>{order.paymentLabel||'—'}</span></div>
//       <div style={S.row}><span style={S.lbl}>Sender No.</span>    <span style={S.val}>{order.senderNumber||'—'}</span></div>
//       <div style={S.row}><span style={S.lbl}>TRX ID</span>        <span style={S.valMono}>{order.trxId||'—'}</span></div>
//       <div style={S.row}><span style={S.lbl}>Amount Paid</span>   <span style={S.valGreen}>৳{deliveryFee} ✓ PAID</span></div>
...
//           <span style={S.lbl}>{item.name} × {item.qty}</span>
//           <span style={S.val}>৳{(item.subtotal ?? (item.price * item.qty))?.toLocaleString()}</span>
//         </div>
//       ))}
...
//       </div>
//       <div style={S.row}>
//         <span style={{...S.lbl, color:'#166534'}}>Delivery Fee (Pre-Paid ✓)</span>
...
//           <div style={S.codSub}>Delivery ৳{deliveryFee} already paid via {order.paymentLabel}</div>
//         </div>
...
//         <span style={{...S.val, color:'#888888'}}>৳{totalAmount?.toLocaleString()}</span>
//       </div>
...
//       >
//         <div
//           className="relative w-full max-w-[620px] rounded-[8px] border border-[#1a1a1a] overflow-hidden
...
//           {/* Info strip */}
//           <div
//             className={`px-5 py-2 text-[11px] flex items-center gap-2 flex-shrink-0
//                            ${mode === "both" ? "bg-green-500/10 text-green-400" : "bg-blue-500/10 text-blue-400"}`}
//           >
//             <svg
//               width="11"
...
//             <div
//               ref={customerRef}
//               style={{ display: mode === "customer" ? "block" : "none" }}
//             >
//               <MemoContent
//                 order={order}
//                 orderDate={orderDate}
...
//             <div
//               ref={bothRef}
//               style={{ display: mode === "both" ? "block" : "none" }}
//             >
//               <MemoContent
//                 order={order}
//                 orderDate={orderDate}
