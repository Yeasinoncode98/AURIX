...
function NavAvatar({ user, profile, size = 30 }) {
  ...
  return (
    <div className="rounded-full flex items-center justify-center font-bold text-white flex-shrink-0 border border-red/40"
      style={{
    ...
    <>
      ...
      >
        <div className="w-full max-w-[1280px] mx-auto flex items-center justify-between relative">
              ...
              <div className="relative" ref={menuRef}>
                <button
                  ...
                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none" className="text-muted"
                    style={{ transform: userMenuOpen ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s ease' }}>
                    <path d="M1 3l4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </button>

                {/* Dropdown */}
                <div className={`absolute right-0 top-full mt-2 w-[200px] rounded-[6px] border border-border overflow-hidden
                  ...
                  style={{ background: '#111', boxShadow: '0 16px 40px rgba(0,0,0,0.6)' }}>
                  {/* User info */}
                  ...
                  <div className="py-1">
                    ...
                    <Link to="/shop" onClick={() => setUserMenuOpen(false)}
                      className="flex items-center gap-3 px-4 py-2.5 text-[12px] text-off hover:text-white hover:bg-white/5
                                 no-underline transition-colors duration-150 cursor-pointer">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
          ...
          {/* Hamburger */}
          <button onClick={toggleDrawer} aria-label="Toggle menu" aria-expanded={drawerOpen}
      ...
      <nav aria-label="Mobile navigation"
        ...
        <div className="px-6 py-4 space-y-3">
          ...
          {user && (
            <button onClick={async () => { setDrawerOpen(false); setHamburgerOpen(false); await logout(); navigate('/') }}
              className="flex items-center justify-center w-full text-[12px] font-semibold tracking-wider2 uppercase
